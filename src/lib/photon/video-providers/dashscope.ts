...
export const dashscopeProvider: VideoProvider = {
  name: "dashscope",
  ...
  async generateVoice(text: string, options?: { voice?: string }): Promise<VoiceResult> {
    ...
    // DashScope TTS returns audio data directly if called with stream/accept header,
    // or a JSON with audio URL for async. We need to handle both.
    ...
    // Binary audio — save to a temp path or return as data URL
    // For now, we save to a predictable temp location
